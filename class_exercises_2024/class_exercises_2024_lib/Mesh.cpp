...
void Mesh::loadTexCoords(const glm::vec2* texCoords, size_t num_verts) {
	assert(num_verts >= _vertices.size());
	_texCoordBuffer.loadElements(_vertices.size(), texCoords);
}

void Mesh::loadNormals(const glm::vec3* normals, size_t num_verts) {
	assert(num_verts >= _vertices.size());
	_normalBuffer.loadElements(_vertices.size(), normals);
}

void Mesh::loadColors(const glm::u8vec3* colors, size_t num_verts) {
	assert(num_verts >= _vertices.size());
